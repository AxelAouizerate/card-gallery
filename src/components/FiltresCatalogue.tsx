...
/**
 * Unique ilot client du catalogue. Il n'affiche aucune carte : il ne fait
 * qu'ecrire l'etat dans l'URL, et c'est le serveur qui filtre et rend la
 * grille. La liste des sets n'est jamais rendue a plat — elle passe par une
 * palette de recherche (280 options ne doivent pas peser dans le HTML).
 */
export default function FiltresCatalogue({ options, base }: { options: Options; base: string }) {
  ...
  const f = useMemo(
    () => lireFiltres(Object.fromEntries(params.entries())),
    [params],
  );
  const actifs = nbFiltresActifs(f);
  ...
  const bascule = (cle: "raretes" | "langues" | "sets", valeur: string) => {
    const courant = f[cle];
    naviguer({
      ...f,
  ...
  const chips: { libelle: string; retirer: () => void }[] = [
    ...
    ...(f.gradation
      ? [{ libelle: f.gradation === "gradee" ? "Gradée" : "Non gradée", retirer: () => naviguer({ ...f, gradation: "" }) }]
      : []),
    ...
    <div className={cn("space-y-3", enCours && "opacity-60 transition-opacity")}>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Sheet open={ouvert} onOpenChange={setOuvert}>
          <SheetTrigger className="inline-flex items-center gap-2 rounded-md border border-amber-500/40 bg-amber-500/15 px-3.5 py-2 text-sm font-medium text-amber-100 hover:bg-amber-500/25">
            <SlidersHorizontal className="h-4 w-4" />
            Filtres
            {actifs > 0 && (
              <span className="rounded-full bg-amber-400 px-1.5 font-mono text-[11px] font-bold text-black">
                {actifs}
              </span>
            )}
          </SheetTrigger>
            ...
            <div className="space-y-6 pb-4">
              <Bloc titre="Set / Extension">
                <Command className="overflow-hidden rounded-md border border-white/10 bg-black/40">
                  <div className="flex items-center border-b border-white/10 px-2">
                    <Search className="h-4 w-4 text-amber-100/40" />
                    <Command.Input
                  ...
                  <Command.List className="max-h-56 overflow-y-auto p-1">
                    <Command.Empty className="px-2 py-3 text-xs text-amber-100/50">
                      Aucun set.
                    </Command.Empty>
                      ...
                      return (
                        ...
                        >
                          <span className="flex items-center gap-2 font-mono uppercase">
                            <Check className={cn("h-3.5 w-3.5", choisi ? "opacity-100 text-amber-300" : "opacity-0")} />
                            {set}
                          </span>
              ...
              <Bloc titre="Prix (€)">
                <div className="flex items-center gap-2">
                  ...
                  <ChampNombre valeur={f.prixMax} placeholder={String(options.prixMax)} onValider={(v) => naviguer({ ...f, prixMax: v })} />
                </div>
              </Bloc>
