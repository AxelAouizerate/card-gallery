...
// (pas une fine ligne de texte discrete). Annonces (rachat, trade,
// receptions, lives...) + liens reseaux sociaux.
export default function BandeauAnnonces() {
  return (
    <div className="bg-gradient-to-r from-amber-600 via-yellow-500 to-amber-600">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-6 gap-y-2 px-4 py-3 text-center sm:justify-between">
        <p className="flex flex-wrap items-center justify-center gap-x-6 gap-y-1.5">
          {ANNONCES.map((msg, i) => (
            <span key={i} className="text-sm font-bold text-black sm:text-base">
              {msg}
            </span>
          ))}
        </p>
